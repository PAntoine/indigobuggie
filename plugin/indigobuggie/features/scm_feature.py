...
class SCMFeature(Feature):
	...
	def initialise(self, tab_window):
		...
		self.active_scm = self.tab_window.getConfiguration('CodeReviewFeature', 'active_scm')
		if self.active_scm is None:
			self.active_scm = ''

		self.supported_scms = self.tab_window.getConfiguration('SCMFeature', 'supported_scms')
		if self.supported_scms is None:
			self.supported_scms = []
	...
	def getDialog(self, settings):
		...
		for scm in beorn_lib.scm.getSupportedSCMs():
			selected = (scm.type == self.tab_window.getConfiguration('SCMFeature', 'active_scm'))
			scm_list.append((selected, scm.type))

			value = scm.type in self.supported_scms
			button_list.append((value, scm.type))

		dialog_layout = [
			beorn_lib.dialog.Element('ButtonList',{'name': 'active_scm',			'title':'Default SCM', 'x':4, 'y':1, 'width':64, 'items': scm_list, 'type': 'single'}),
			...
			beorn_lib.dialog.Element('ButtonList',{'name': 'supported_scms',		'title':'Supported SCMs', 'x':4, 'y':len(scm_list) + 6, 'width':64, 'items': button_list, 'type': 'multiple'}),
	...
	def resultsFunction(self, settings, results):
		if settings.getName() == 'P4':
			...
			for index, scm in enumerate(beorn_lib.scm.getSupportedSCMs()):
				if results['active_scm'][index]:
					self.active_scm = scm.type

			self.tab_window.setConfiguration('SCMFeature', 'active_scm', self.active_scm)

			if 'poll_period' in results and int(results['poll_period']) != int(self.poll_period):
				self.poll_period = float(results['poll_period'])
				self.tab_window.setConfiguration('SCMFeature', 'poll_period', int(self.active_scm))
			...
			if 'supported_scms' in results:
				ll = []
				supported_list = beorn_lib.scm.getSupportedSCMs()

				for index, item in enumerate(results['supported_scms']):
					if item:
						ll.append(supported_list[index].type)

				li = ','.join(ll)
				if li != self.supported_scms:
					self.supported_scms = li
					self.tab_window.setConfiguration('SCMFeature', 'supported_scms', li)
	...
	def getSettingsMenu(self):
		...
		for engine in beorn_lib.scm.getSupportedSCMs():
			if (supported is None or engine.type in supported):
